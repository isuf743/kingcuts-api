...
if($action === 'cancel_booking_client' && $method === 'POST'){
    ...
    // Funksion normalizimi - nxjerr vetem shifrat
    function normalizePhone($p){
        $p = preg_replace('/[^0-9]/', '', $p); // vetem shifra
        if(substr($p,0,3) === '355') $p = substr($p,3); // hiq 355
        if(substr($p,0,1) === '0')   $p = substr($p,1);  // hiq 0 fillestare
        return $p; // kthej vetem 9 shifrat e fundit p.sh. 691234567
    }
    $phone_clean = normalizePhone($phone);
    if(strlen($phone_clean) < 9) respond(array('error'=>'Numri i telefonit nuk eshte valid!'), 400);
    ...
    // Normalizoj numrin nga DB dhe krahaso vetem shifrat
    $db_phone = normalizePhone($row['client_phone']);

    if($phone_clean !== $db_phone){
        respond(array('error'=>'Numri i telefonit nuk perputhet!'), 400);
    }
